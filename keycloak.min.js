...
(function (window, undefined) {
    ...
    var Keycloak = function (config) {
        ...
        kc.createLoginUrl = function (options) {
            ...
            if (kc.pkce) {
                url += '&code_challenge=' + encodeURIComponent(kc.code_challenge);
                url += '&code_challenge_method=' + encodeURIComponent(kc.code_challenge_method);
            }
            ...
            req.onreadystatechange = function () {
                if (req.readyState == 4) {
                    if (req.status == 200) {
                        ...
                        promise.setError({status: req.status, message: req.statusText});
                    }
                }
            }
        ...
        kc.isTokenExpired = function (minValidity) {
            if (!kc.tokenParsed || (!kc.refreshToken && kc.flow != 'implicit' )) {
                throw new Error({status: 440, message: 'Not authenticated'});
            }
                        ...
                        req.onreadystatechange = function () {
                            if (req.readyState == 4) {
                                if (req.status == 200) {
                                    ...
                                    for (var p = refreshQueue.pop(); p != null; p = refreshQueue.pop()) {
                                        p.setError({status:440, error:req.response});
                                    }
                                }
                            }
                        };
        ...
        function processCallback(oauth, promise) {
            ...
            if ((kc.flow != 'implicit') && code) {
                ...
                if (kc.pkce) {
                    params += "&code_verifier=" + kc.code_verifier;
                    cleanVerifier();
                }
                var url = getRealmUrl() + '/protocol/openid-connect/token';
                ...
                req.onreadystatechange = function () {
                    if (req.readyState == 4) {
                        if (req.status == 200) {
                            ...
                            kc.onAuthError && kc.onAuthError();
                            promise && promise.setError({status: req.status, message: req.statusText});
                        }
                    }
                };
        ...
        function cryptoEncodeSHA256b64(input, ieCb) {
            var buffer = new Uint8Array(input.split('').map(function (l) {
            ...
            if (isIE) {
                CryptoSubtle().digest({name: 'SHA-256'}, buffer).oncomplete = function (event) {
                    var hash = String.fromCharCode.apply('', new Uint8Array(event.currentTarget.result));
                    ieCb(btoa(hash));
                };
            ...
            return CryptoSubtle().digest({name: 'SHA-256'}, buffer).then(function (result) {
                var hash = String.fromCharCode.apply('', new Uint8Array(result));
                return btoa(hash);
            })
        ...
        function cleanHash(hash) {
            ...
            hash = replaceAll(hash, '+', '-');
            hash = replaceAll(hash, '/', '_');

            return hash;
        }

        function s256Encode(input) {
            // var output = new Hashes.Base64().encode(new Hashes.SHA256().hex(kc.code_verifier));
            ...
            if (isIE) {
                return {
                    then: function (cb) {
                        cryptoEncodeSHA256b64(input, function (hash) {
                            console.log(hash);
                            cb(cleanHash(hash));
                        });
                    }
            ...
            return cryptoEncodeSHA256b64(input).then(function (hash) {
                console.log(hash);
        ...
        function createVerifier() {
            var output = localStorage.getItem("kc.oidc.pkce");
            if (!output) {
                output = createUUID() + "-" + createUUID();
                output = replaceAll(output, '', '')
                localStorage.setItem("kc.oidc.pkce", output)
            }
            return output;
        }

        function cleanVerifier() {
            localStorage.removeItem("kc.oidc.pkce");
        }
        ...
        function loadConfig(url) {
            ...
            if (typeof url === 'object') {
                ...
                kc.pkce = config['enable-pkce'] || false;

                if (kc.pkce) {
                    kc.code_verifier = createVerifier();

                    s256Encode(kc.code_verifier).then(function (code) {
                        kc.code_challenge = code;
                        kc.code_challenge_method = 'S256';
                        console.log(kc.code_challenge);
                        promise.setSuccess();
                    });
            ...
            if (type == 'cordova') {
                ...
                return {
                    login: function (options) {
                        ...
                        var o = 'location=no,zoom=no,toolbar=no';
